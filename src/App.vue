...
<div id="vue">
    ...
    <div id="form" style="margin-inline-start: 50px;margin-inline-end: 50px;">
        ...
        <el-input style="margin-top: 12px;" v-model="textarea" :rows="8" type="textarea" placeholder="请输入描述" />
        <div>
    ...
    <el-card v-for="o in comments" :key="o" class="box-card" shadow="hover">
        ...
        <div style="margin-top: 5px;" v-if="o.urlmessage">
            <el-button style="padding: 20px 20px;" type="primary" @click="Dowloadwav(o.url)">下载wav<el-icon class="el-icon--right">
                    <Download />
                </el-icon>
            </el-button>
        </div>
...
export default {
    created() {
        ...
        if (this.$cookies.isKey('token') == false) {
            this.num = 1
            this.comments = [{
                url: 'https://github.com/login/oauth/authorize/?client_id=Iv1.35848ad28173c501',
                urlmessage: false,
                message: '登录后即可查看历史记录\n'
            }]
        }
        if (window.location.href.includes("code")) {
            ...
            var config = {
                method: 'get',
                url: 'https://api.xuqinyang.top/https://github.com/login/oauth/access_token?client_id=Iv1.35848ad28173c501&client_secret=c384f98bfdfc1c992f72934f7e0250f7d303611b&code=' + this.id,
                headers:{accept: 'application/json'}
            };
            var _this = this
            axios(config)
                .then((response) => {
                    if (response.data.access_token.includes("gh")) {
                        _this.$cookies.set("token", response.data.access_token, {
                            expires: "30D"
                        });
                    }
                    fetch(
    ...
    watch: {
        login: function () {
            ...
            fetch(
                    "https://api.github.com/repos/xqy2006/musicgen/issues?state=closed&per_page=100&creator=" + login, {
                        headers: {
                ...
                .then(json => {
                    ...
                    json.forEach(comment => {
                        this.comments.push({
                            url: 'https://xqy2006.github.io/musicgen/music/' + comment["number"],
                            urlmessage: true,
    ...
    methods: {
        ...
        Login() {
            if (this.$cookies.isKey('token')) {
                ...
                if (typeof this.textarea == 'undefined' || this.textarea == '') {
                    window.location.href = "https://github.com/login/oauth/authorize/?client_id=Iv1.35848ad28173c501"
                } else {
                    window.location.href = "https://github.com/login/oauth/authorize/?client_id=Iv1.35848ad28173c501&state=" + this.textarea
                }
            }
        },
        Logout() {
            ...
            if (this.textarea == '') {
                ElNotification({
                    ...
                    if (this.$cookies.isKey('token')) {
                        ...
                        fetch(
                                "https://api.github.com/repos/xqy2006/musicgen/issues", {
                                    headers: {
